...
public class Steganography {
    ...
    public static void cacherMessage(File fichierEntree, String message) throws Exception {
        ...
        File dossierSortie = new File("imagesResultat"); // Créer un objet File avec le nom du dossier
        ...
        String nomFichierSortie = nomFichierOrigine.substring(0, nomFichierOrigine.lastIndexOf('.')) + "_cacher.png"; // Créer le nom du fichier de sortie
        File fichierSortie = new File(dossierSortie, nomFichierSortie); // Créer un objet File avec le nom du fichier de sortie

        // Enregistrer l'image dans le dossier imagesResultat avec le nouveau nom
        ImageIO.write(image, "png", fichierSortie);
        System.out.println("Message caché dans l'image avec succès dans " + fichierSortie.getAbsolutePath()); // Afficher un message de succès
    }
    ...
    public static String lireMessage(BufferedImage image) {
        ...
        for (int i = 0; i < 32; i++) {
            int pixel = image.getRGB(i % image.getWidth(), i / image.getWidth()); // Récupérer le pixel
            int bleu = pixel & 0xFF; // Extraire la composante bleue
            longueurMessage = (longueurMessage << 1) | (bleu & 1);  // Ajouter le bit de poids faible à la longueur du message
        }
        ...
        for (int i = 0; i < longueurMessage * 8; i++) {
            int pixel = image.getRGB((i + 32) % image.getWidth(), (i + 32) / image.getWidth()); // Récupérer le pixel
            int bleu = pixel & 0xFF; // Extraire la composante bleue
            messageOctets[i / 8] = (byte)((messageOctets[i / 8] << 1) | (bleu & 1)); // Ajouter le bit de poids faible au message
        }
