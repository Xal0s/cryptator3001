...
public class LFSR {
    ...
    private LFSR(int[] positionsXOR, int etatInitial, int tailleBits) {
        ...
        this.etat = etatInitial & ((1 << tailleBits) - 1); // Limite l'état initial au nombre de
                                                           // bits
        this.masque = (1 << tailleBits) - 1; // Crée un masque pour maintenir l'état dans le nombre
                                             // de bits
    }
    ...
    private int etape() {
        ...
        etat = ((etat >> 1) | (nouveauBit << (Integer.SIZE - 1))) & masque; // Décale et limite aux
                                                                            // bits
        return nouveauBit;
    }
    ...
    private String obtenirEtatBinaire(int tailleBits) {
        return String.format("%" + tailleBits + "s", Integer.toBinaryString(etat)) // Convertit
                                                                                   // l'état en
                                                                                   // binaire
    ...
    public static int executerLFSR(int[] positionsXOR, int etatInitial, int tailleBits,
            int nombreEtapes, int bitsAGenerer) {
        LFSR lfsr = new LFSR(positionsXOR, etatInitial, tailleBits); // Crée une instance de LFSR
        // System.out.println("État initial : " + lfsr.obtenirEtatBinaire(tailleBits)); // Affiche
        // l'état initial

        // Exécute les étapes et affiche l'état
        for (int i = 0; i < nombreEtapes; i++) {
            int bit = lfsr.etape(); // Effectue une étape
            // System.out.println("Étape " + (i + 1) + " - Nouveau bit : " + bit); // Affiche le
            // nouveau bit
            // System.out.println("État actuel : " + lfsr.obtenirEtatBinaire(tailleBits)); //
        ...
        // System.out.println("Nombre aléatoire généré sur " + bitsAGenerer + " bits : " +
        // valeurAleatoire); // Affiche le nombre généré
